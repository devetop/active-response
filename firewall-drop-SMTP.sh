...
ipset_active_ban() {
        $(iptables -C INPUT_imunify360 -m set --match-set ossec.ipv4.blacklist_SMTP src -p tcp -m multiport --dports 25,465,587 -j DROP)
        if [ $? -eq 0 ]
        then
          $(ipset test ossec.ipv4.blacklist_SMTP $IP -exist > /dev/null 2>&1)
          [ $? -ne 0 ] && ipset add ossec.ipv4.blacklist_SMTP ${IP}
        else
          ipset create ossec.ipv4.blacklist_SMTP hash:net timeout 600 maxelem 100000 -exist
          iptables -A INPUT_imunify360 -m set --match-set ossec.ipv4.blacklist_SMTP src -p tcp -m multiport --dports 25,465,587 -j DROP
          ipset add ossec.ipv4.blacklist_SMTP ${IP} -exist
        fi
}
...
if [ "X${UNAME}" = "XLinux" ] && [[ " $config_value " =~ $value ]]; then
   if [ "x${ACTION}" = "xadd" ]; then
      ...
      ipset del ossec.ipv4.blacklist_SMTP ${IP} -exist
   else
      :
   fi
   exit 0;
  ...
  pam_smtp_state=$( cat "${config}" | grep exim_dovecot_protection | grep false )

   if [ "x${ACTION}" = "xadd" ] && [[ "$pam_smtp_state" =~ "false" ]]; then
     # note, that RULE_ID become 200000+RULE_ID because we increasing severity
     echo '{"severity": 7, "name": "Preventing SMTP brute force attempt.", "timestamp": '$DT', "attackers_ip": "'$IP'", "plugin_id": "ossec", "method": "INCIDENT", "message": "[Active Response] is disabled for rule '$RULE_ID'. Greylisting '$IP'", "rule": '$(($RULE_ID+200000))' }' | nc -w1 -U /var/run/defence360agent/generic_sensor.sock.2
     echo "$(date '+%b %d %T') [INFO] AR is disabled, sent alert for smtp bruteforce greylisting: $IP" >>${LOG_FILE}
   else
